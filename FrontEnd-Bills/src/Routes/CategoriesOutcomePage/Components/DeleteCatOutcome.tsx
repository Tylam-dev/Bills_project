...
const DeleteCatOutcome = () => {
    const [open] = useState<boolean>(true)
    const { idCatOut, nameCatOut } = useParams()
    const {deleteCatOutcome} = CatOutcomeHook()
    ...
    const submitDeleteCatIncome = async() => {
        if (idCatOut ){
            try{
                await deleteCatOutcome(idCatOut)
                handleCloseReload()
            }catch(error){
                console.error(error)
            }
    ...
    return(
        ...
        <Box sx={style}>
            ...
            <strong>{nameCatOut}</strong> category will be deleted, are you sure?
            </Typography>
